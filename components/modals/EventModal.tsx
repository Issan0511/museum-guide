...
export default function EventModal({ open, onClose, event, lang = "ja" }: EventModalProps) {
  if (!open) return null;

  const t = getTranslations(lang);
  const eventName = pickLang(event.name, lang) ?? "";
  const eventDetail = pickLang(event.detail, lang) ?? "";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <Card className="w-full max-w-md max-h-[80vh] overflow-y-auto relative">
        ...
        <CardContent className="p-4 space-y-3">
          ...
          <div>
            <p className="text-sm text-neutral-600 whitespace-pre-wrap">{eventDetail}</p>
          </div>
          <div>
